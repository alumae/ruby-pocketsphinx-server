...
* Rack
* Unicorn
* Additional Ruby modules: gdbm, rack/throttle
* Pocketsphinx
...
Now, let the server download and compile it:
 ...
 curl -vv  http://localhost:8080/fetch-jsgf?lm=http://www.example.com/robot.jsgf
...
A GF grammar must be compiled into a .pgf file. To upload it to the server, use the fetch-pgf API call, e.g.:
  ...
  curl "http://bark.phon.ioc.ee/speech-api/v1/fetch-pgf?url=http://kaljurand.github.com/Grammars/grammars/pgf/Calc.pgf&lang=Est"
...
Multiple output languages can be used (not tested), by using comma-speperated values: "..&output-lang=App,App2"

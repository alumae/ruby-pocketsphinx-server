#! /usr/bin/env python

import sys

from math import exp

if __name__ == '__main__':
  arcs = []
  for l in sys.stdin:
    ss = l.split()
    if len(ss) == 5:
      arcs.append((int(ss[0]), int(ss[1]), ss[2], exp((-float(ss[4])))))
    elif len(ss) == 4:
      arcs.append((int(ss[0]), int(ss[1]), ss[2], 1))
    elif len(ss) == 2:
      arcs.append((int(ss[0]), -1, "", exp(-(float(ss[1])))))
    elif len(ss) == 1:
      arcs.append((int(ss[0]), -1, "", 1))
    else:
      print >>sys.stderr, "WARNING: strange FSG line: ", l
  
  max_state = max([a[1] for a in arcs])
  
  final_state_id = max_state + 1
  
  # HACK: make all weights 1.0. This is stupid, but seems to make the recognition more robust
  for a in arcs:
    a = (a[0], a[1], a[2], 1.0)
  
  print "FSG_BEGIN <test>"
  print "NUM_STATES", max_state + 2
  print "START_STATE 0"
  print "FINAL_STATE", final_state_id
  
  for a in arcs:
    print "TRANSITION", a[0], a[1] == -1 and final_state_id or a[1], "%7.5f" % min(1.0, a[3]), a[2]

  print "FSG_END"
